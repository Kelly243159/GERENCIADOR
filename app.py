...
# -------------------------------------------------
# E-mail via Outlook (Windows) - MODIFICADO
# -------------------------------------------------
def enviar_email_outlook(destinatario: str, assunto: str, body_html: str):
    """
    Envia e-mail utilizando o Outlook instalado (win32com).
    - Requer Windows + Outlook instalado + biblioteca pywin32 (pip install pywin32)
    - Envia imediatamente (Send) usando a conta padrão do Outlook
    """
    try:
        ...
        mail.Send()  # envia direto; use .Display() se quiser revisar
        return True, "E-mail enviado com sucesso via Outlook"
    except Exception as e:
        return False, f"Falha ao enviar via Outlook: {e}"
    ...
    try:
        ...
        if vencidos > 0:
            for _, row in df[df['Status'] == 'Vencido'].iterrows():
                ...
                card_content = [
                    Div(
                        ...
                        Button("📤 Enviar E-mail", cls="btn",
                               ...
                               hx_post=f"/envia-email-individual/{data_id}/vencido/{email_hash}",
                               hx_target=f"#res_vencido_{email_hash}",
                               hx_swap="outerHTML") if email and '@' in email else
                ...
                card_content.append(Div(id=f"res_vencido_{email_hash}"))
        ...
        cards_a_vencer = []
        if avencer > 0:
            for _, row in df[df['Status'] == 'A vencer'].iterrows():
                email = (row.get('Email') or '').strip()
                empresa = row.get('Empresa', '') or 'Não informado'
                vencimento = row.get('Vencimento', '') or 'Não informado'
                ...
                card_content = [
                    Div(
                        ...
                        Button("📤 Enviar E-mail", cls="btn",
                               style="padding:6px 12px;font-size:12px",
                               hx_post=f"/envia-email-individual/{data_id}/avencer/{email_hash}",
                               hx_target=f"#res_avencer_{email_hash}",
                               hx_swap="outerHTML") if email and '@' in email else
                        Span("E-mail inválido", style="color:var(--muted);font-size:12px"),
                        cls="cert-header"
                    ),
                ...
                card_content.append(Div(id=f"res_avencer_{email_hash}"))
        ...
        return Div(
            ...
            Div(
                ...
                P("Envie os avisos diretamente pelo Outlook."),
                Div(
...
def envia_emails_lote(request, data_id: str, tipo: str):
    ...
    if certificados.empty:
        return Div(P(f"✅ Nenhum certificado {tipo.replace('_', ' ')} com e-mail válido."), cls="glass", id=target_id)

    enviados, erros = 0, 0
    linhas = []
    for _, cert in certificados.iterrows():
        email = (cert.get('Email') or '').strip()
        if not email or '@' not in email:
            continue
        assunto = f"{assunto_base}{cert.get('Empresa', 'Cliente')}"
        html = funcao_corpo(cert.get('Empresa', 'Cliente'), cert.get('Vencimento', 'Data não informada'))
        ok, msg = enviar_email_outlook(email, assunto, html)
        if ok:
            enviados += 1
            linhas.append(
                Tr(Td("✅"), Td(cert.get('Empresa', '')), Td(cert.get('Vencimento', '')), Td(email), Td("Enviado")))
        else:
            erros += 1
            linhas.append(Tr(Td("❌"), Td(cert.get('Empresa', '')), Td(cert.get('Vencimento', '')), Td(email), Td(msg)))
...
@app.post("/envia-email-individual/{data_id}/{tipo}/{email_hash}")
def envia_email_individual(request, data_id: str, tipo: str, email_hash: str):
    df = current_data.get(data_id)
    if df is None:
        return Div(Span("❌ Dados não encontrados", style="color:#ef4444;font-weight:600"))

    if tipo == "vencido":
        status_filtro = 'Vencido'
        assunto_base = "AVISO: Certificado Digital Vencido - "
        funcao_corpo = corpo_email_vencido
    elif tipo == "avencer":
        status_filtro = 'A vencer'
        assunto_base = "ALERTA: Certificado Digital Vence em Breve - "
        funcao_corpo = corpo_email_a_vencer
    else:
        return Div(Span("❌ Tipo de e-mail inválido", style="color:#ef4444;font-weight:600"))

    for _, cert in df[df['Status'] == status_filtro].iterrows():
        email = (cert.get('Email') or '').strip()
        if email and hash(email) == int(email_hash):
            assunto = f"{assunto_base}{cert.get('Empresa', 'Cliente')}"
            html = funcao_corpo(cert.get('Empresa', 'Cliente'), cert.get('Vencimento', 'Data não informada'))
            ok, msg = enviar_email_outlook(email, assunto, html)
            if ok:
                return Div(Span("✅ E-mail enviado!", style="color:#10b981;font-weight:600"))
            else:
                return Div(Span("❌ Erro: " + msg, style="color:#ef4444;font-weight:600"))

    return Div(Span("❌ Certificado não encontrado", style="color:#ef4444;font-weight:600"))
